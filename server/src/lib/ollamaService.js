...
COUNTS:
- Total deal records: ${crmContext.counts.totaldonors}
- Closed donors: ${crmContext.counts.closeddonorsCount}
- Standard Pipeline (open) donors: ${crmContext.counts.standardPipelineCount}
...
CONVERSION (closed/won donors):
- All-time total conversion: ${formatINR(crmContext.conversion.allTime.amount)}, ${crmContext.conversion.allTime.donors} donors, ${crmContext.conversion.allTime.count} donors
...
PIPELINE (open/Standard Pipeline donors):
...
`
  ...
  const response = await ollama.chat({
    ...
    messages: [
      {
        role: "system",
        content: `You are Smart AI Assistant, helping users understand their Zoho Bigin CRM data.
...
Never invent numbers or names not present in it. "Conversion" means closed/won donors; "Pipeline" means open/Standard Pipeline donors — these are NOT based on the raw "Stage" field.

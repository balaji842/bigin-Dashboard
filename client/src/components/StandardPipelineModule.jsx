...
const TABLE_COLUMNS = [
  ...
  { key: "spoc", label: "SPOC" },
  { key: "expectedMonth", label: "Expected Month" },
...
export default function StandardPipelineModule() {
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    fetch(`/api/crm-analysis/standard-pipeline?fy=${encodeURIComponent(fy)}`)
      .then((r) => {
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <KpiCard
          label={`Standard Pipeline — FY ${data.fy}`}
          amount={data.totals.amount}
          donors={data.totals.donors}
          accent="pink"
        />
        <KpiCard
          label="Standard Pipeline — All Time"
